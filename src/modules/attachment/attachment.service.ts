...
export class AttachmentService {
  ...
  constructor(
    ...
    // Construct base URL for serving files. Adjust port/domain as needed from config.
    ...
    // Ensure APP_HOST does not have a trailing slash if APP_PORT is present
    const hostPart = appPort ? appHost.replace(/\/$/, '') : appHost;
    this.baseUrl = appPort ? `${hostPart}:${appPort}/static` : `${hostPart}/static`;
  ...
  async upload(
    ...
    user: Types.ObjectId | string, // Add user param
  ): Promise<Attachment> {
    if (!file) {
    ...
    const attachmentData = {
      ...
      as,
      createdAt: new Date(),
      user: typeof user === 'string' ? new (require('mongoose').Types.ObjectId)(user) : user, // Set user
    };
